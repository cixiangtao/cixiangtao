...
**The 桌面图标轨道规则.** 1024px 以上把第三方与快捷入口垂直对齐在舞台左上，窗口初始构图为其保留独立横向空间。
...
- **导航：** 只承载站内入口，使用头像与共享的文件夹、文稿、九宫格像素图标；当前栏目以文件黄底和虚线轮廓反馈，中文名称通过可访问标签和悬停提示表达。
- **信息：** 右侧只显示当前栏目与在线状态，不放置无交互装饰或 GitHub 等第三方导航；第三方入口使用桌面快捷图标或内容中的明确外链。
...
- **图标实现：** `ModuleIcon` 统一维护系统工具栏与窗口标题栏的三类像素图标，并通过 toolbar、window 两个尺寸保持各自的光学清晰度。
