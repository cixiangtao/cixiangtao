...
description: 一台以写作为主屏、以项目和作者身份为辅助窗口的中英双语个人桌面。
colors:
  desktop: "#6670c9"
  desktop-dot: "#5962b1"
  paper: "#fffefa"
...
- 默认使用英文界面，`/zh/` 提供完整中文界面；项目名、技术名词和 GitHub Issue 正文保留其固有写法。
- 桌面背景可在经典紫蓝点阵、水色波纹和午夜星点之间切换；背景只改变桌面环境，不改变窗口、字体和内容配色。换肤只在 1024px 以上的桌面端提供操作入口。
...
**The 双语界面规则.** 每个界面的导航、状态和说明必须保持同一种语言；英文为默认语言，中文位于 `/zh/`，不混用装饰性外语。桌面模块名称统一由 `sections` 提供，并只使用单词名称：英文为 `About`、`Projects`、`Writing`，中文为“关于”、“项目”、“文章”。
...
**The 项目时效规则.** 项目窗口展示未归档、非 Fork 的公开仓库，但排除 `cixiangtao` Profile 仓库；项目按最近代码推送时间倒序排列，桌面窗口内部滚动，窄屏随页面完整展开。

**The 桌面图标轨道规则.** 1024px 以上把关于我、项目和文章三个站内窗口入口垂直对齐在舞台左上，窗口初始构图为其保留独立横向空间；窄屏改为窗口上方的横向入口行。
...
直角矩形是主导形态。窗口、按钮、标签、头像和图标均使用零圆角与一像素墨黑边框；标题栏左侧窗口图标、中央条纹与右侧最小化、最大化、关闭控件共同形成可识别的 MacWindow 轮廓。圆形仅保留给状态点，像素图形允许使用硬边内阴影模拟体积。
...
- **职责：** 首页头像与当前位置位于左侧；右侧只承载皮肤、语言等全局功能和按访客本地时区显示的系统时间。站外导航归入关于我窗口，站内窗口入口全部交给桌面图标。
- **信息：** 当前栏目紧邻左侧首页头像，站内窗口切换时同步更新；中间留白分隔页面上下文与全局工具。
- **移动端：** 720px 以下隐藏当前位置文字并缩短组间距；站内入口不回流工具栏，而是在窗口上方变为横向快捷入口。
- **语言切换：** 右侧功能组提供持续可见的原生语言下拉选择器；控件始终显示当前语言 `English` 或 `中文`，切换时保持当前首页、窗口锚点或文章详情上下文，并主动避开另一语言路由的旧 HTML 缓存；临时缓存参数在页面加载后立即清理，`hreflang` 继续暴露对应语言页面。
- **桌面背景：** 1024px 以上时，语言控件左侧显示当前背景缩略图的显示器按钮；点击按钮或在桌面空白处右键均打开同一个上下文菜单。“皮肤 / Skin”为一级菜单，Classic、Aqua、Midnight 为二级单选项，当前项使用勾选标记；选择写入浏览器本地存储并在中英文页面间保持。移动端隐藏显示器按钮且不接管原生右键。

### Context Menu

- **形态：** 通用直角菜单使用文稿白表面、一像素墨黑边框与硬偏移阴影；二级菜单默认向右展开，贴近视口边缘时自动翻转或上移。
- **交互：** 支持悬停、点击、方向键、Enter、Space 与 Esc；点击外部、页面滚动、视口缩放或窗口失焦时关闭。
- **边界：** 只在桌面空白区域阻止浏览器原生右键；窗口、桌面图标、链接、表单与文章内容保留原生行为。

### MacWindow

- **形态：** 直角文稿表面、一像素墨黑边框、无模糊右下硬阴影。
- **标题栏：** 五列网格容纳语义模块图标、条纹、居中标题、条纹和右侧三控件组；关于我、项目和文章分别使用个人资料、文件夹与文稿图标，最小化、最大化、关闭分别使用黄、青、珊瑚色。
...
- **状态：** 锚点命中或直接点击面板时，窗口提升到所有内容窗口之上并增加文件黄外框，同时以无滚动的方式同步地址栏锚点；1024px 以上可按住标题栏在桌面范围内拖动，拖动时使用抓取光标和更深的硬阴影反馈。站内窗口可通过珊瑚色关闭控件收起，并由对应桌面图标重新打开；窄屏仍保持稳定文档流；系统工具栏始终保持最高层。

### Desktop Shortcut

- **形态：** 48px 直角像素图标配合居中标签，图标使用文稿白表面与硬阴影，标签固定为白色并使用一像素深色文字阴影，以便在紫蓝桌面上保持清晰。
- **语义：** 关于我使用经典个人资料剪影，项目与文章分别使用文件夹和文稿。
- **用途：** 承载关于我、项目和文章三个站内窗口入口；1024px 以上显示为独立的左侧竖向轨道，单击选中图标，双击打开或置顶对应窗口；窄屏显示为窗口上方的横向入口行，并保留单击打开以适配触屏与键盘操作。
...
- **站外入口：** GitHub 与邮箱归入关于我窗口底部，默认仅显示 GitHub 官方品牌图标与信封图标；鼠标悬停或键盘聚焦时显示对应名称，并保留完整可访问名称；系统栏不承担站外导航。
