...
桌面首页以 1480px 的最大舞台承载有明确主次的多窗口构图：文章窗口约占舞台七成并位于前景，项目窗口位于右后方，关于我缩在左上。舞台会填满菜单栏与页脚之间的可用视口，页脚在短页面上贴合底部；顶部图标工具栏固定在页面顶部，桌面快捷图标只在 1024px 以上出现；窗口层级通过位置、尺寸和重叠表达，而不是平均排列。
...
- **导航：** 只承载站内入口，使用头像、文件夹、文稿与九宫格像素图标；当前栏目以文件黄底和虚线轮廓反馈，中文名称通过可访问标签和悬停提示表达。
- **信息：** 右侧显示当前栏目、在线状态和系统装饰控件，不放置 GitHub 等第三方导航；第三方入口使用桌面快捷图标或内容中的明确外链。
- **移动端：** 保留核心图标与在线状态，隐藏当前栏目文字和分隔符，仍维持单行工具栏。
