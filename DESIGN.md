...
- **导航：** 首页头像始终位于左侧；唯一的 GitHub 外链与在线状态组成右侧工具组，站内窗口入口全部交给桌面图标。
- **信息：** 当前栏目紧邻左侧首页头像，站内窗口切换时同步更新；中间留白分隔页面上下文与全局工具。
- **移动端：** 720px 以下隐藏当前位置文字并缩短组间距；站内入口不回流工具栏，而是在窗口上方变为横向快捷入口。
- **语言切换：** GitHub 入口左侧提供持续可见的原生语言下拉选择器；控件始终显示当前语言 `English` 或 `中文`，切换时保持当前首页、窗口锚点或文章详情上下文，并主动避开另一语言路由的旧 HTML 缓存；临时缓存参数在页面加载后立即清理，`hreflang` 继续暴露对应语言页面。
...
- **语义：** 关于我使用经典个人资料剪影，项目与文章分别使用文件夹和文稿；GitHub 在系统栏使用官方轮廓置于黑白应用图标框内，优先保证识别性。
