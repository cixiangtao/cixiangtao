...
桌面首页以 1480px 的最大舞台承载有明确主次的多窗口构图：文章窗口约占舞台七成并位于前景，项目窗口位于右后方，关于我缩在左上。顶部图标工具栏固定在页面顶部，桌面快捷图标只在 1024px 以上出现；窗口层级通过位置、尺寸和重叠表达，而不是平均排列。
...
- **导航：** 使用头像、文件夹、文稿、终端与九宫格像素图标；中文名称通过可访问标签和悬停提示表达。
- **移动端：** 保留核心图标，隐藏在线文字与分隔符，仍维持单行工具栏。
...
- **状态：** 锚点命中或直接点击面板时，窗口提升到所有内容窗口之上并增加文件黄外框，同时以无滚动的方式同步地址栏锚点；系统工具栏始终保持最高层。
